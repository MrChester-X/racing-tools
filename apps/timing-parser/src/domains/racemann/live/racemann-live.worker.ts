...
import { RacemannLiveConfig } from './racemann-live.config';
...
/** Tracks a single racemann subdomain: polls /home/last and streams laps over SignalR. */
export class RacemannLiveWorker {
  private readonly logger: Logger;
  private readonly parser = new RacemannLiveParser();
  ...
  constructor(
    private readonly config: RacemannLiveConfig,
    private readonly client: RacemannClient,
    private readonly timing: TimingService,
  ) {
    this.logger = new Logger(`${RacemannLiveWorker.name}:${config.kartodromId}`);
  }

  start(): void {
    this.logger.log(
      `Starting live worker: subdomain=${this.config.subdomain} pollMs=${this.config.homeLastPollMs}`,
    );
